...
const current = ref("1986");
</script>

<template>
  <div class="w-full">
    <div v-for="(item, index) in histories[current]" class="">
      <h1
        v-if="!index"
        class="text-2xl max-w-[720px] ml-auto mr-auto w-full text-center my-8"
      >
        {{ item }}
      </h1>
      <p
        v-else
        class="max-w-[720px] text-lg leading-10 ml-auto mr-auto w-full text-left mb-6"
        v-html="markedText(item)"
      ></p>
    </div>
  </div>
  <div class="pt-32 pr-32 timeline fixed right-0">
    <Timeline class="z-index-200" mode="right">
      <Timeline-item>Lịch sử hình thành</Timeline-item>
      <Timeline-item class="cursor-pointer" @click="current = '1945'">
        <template v-if="current === '1945'" #dot
          ><clock-circle-outlined style="font-size: 16px"
        /></template>
        Trước 1945</Timeline-item
      >
      <Timeline-item class="cursor-pointer" @click="current = '1986'">
        <template v-if="current === '1986'" #dot
          ><clock-circle-outlined style="font-size: 16px"
        /></template>
        1945 đến 1986</Timeline-item
      >
      <Timeline-item class="cursor-pointer" @click="current = 'now'">
        <template v-if="current === 'now'" #dot
          ><clock-circle-outlined style="font-size: 16px"
        /></template>
        1986 đến nay
      </Timeline-item>
...
<style scoped>
.timeline:deep(.ant-timeline-item) {
  font-size: 16px;
}
</style>
